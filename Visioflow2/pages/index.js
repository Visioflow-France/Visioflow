...
<style>
:root{
  --bg:#ffffff;--bg-alt:#f4f5f8;
  --bord:rgba(0,0,0,.08);--bord-md:rgba(0,0,0,.11);
  --text:#111827;--text2:#6b7280;--text3:#9ca3af;
  --sh:0 2px 12px rgba(0,0,0,.08);
}
@media(prefers-color-scheme:dark){
  :root{
    --bg:#141724;--bg-alt:#1e2235;
    --bord:rgba(255,255,255,.08);--bord-md:rgba(255,255,255,.12);
    --text:#f1f5f9;--text2:rgba(255,255,255,.5);--text3:rgba(255,255,255,.3);
...
(function(){
  ...
  var light = '#f4f5f8';
  var darkBg = '#1e2235';
  var bg = dark ? darkBg : light;

  var panelSelectors = [
    '.vf-pp-wrap','.vf-pp-cbar','.pp-city-item','.pp-menu-item',
    '.pp-radio-card','.fr-section-head','.vf-pp-feat',
    '.vf-auth-card','.ec-card','.vf-auth-tabs',
    '.form-section','.form-wrap','.form-block','.form-card'
  ];
  var inputSelectors = [
    'input:not([type=checkbox]):not([type=radio]):not([type=range]):not([type=color]):not([type=file])',
    'textarea','select',
    '.vf-form-input','.vf-pp-inp','.vf-af-i','.cl-field input','.login-field input'
  ];

  function applyOpaque(){
    panelSelectors.forEach(function(sel){
      document.querySelectorAll(sel).forEach(function(el){
        el.style.setProperty('background', dark ? darkBg : '#ffffff', 'important');
      });
    });
...
function applyConfigToHTML(html, config) {
  if (!config) return html + OPAQUE_CSS
  let h = html
...
window.doAdminLogin = async function(){
  ...
  if(email !== ADMIN_EMAIL.toLowerCase()){
    errT.textContent = 'Accès réservé à l’administrateur.';
    err.classList.add('show');
    card.classList.add('login-shake');
    setTimeout(()=> card.classList.remove('login-shake'), 400);
    return;
  ...
  try {
    let fbAuth;
    try { fbAuth = firebase.auth(); } catch(e2) { fbAuth = null; }
...
/* ===== ASSISTANCE PHONES ===== */
const FIRESTORE_ASSIST_URL = 'https://firestore.googleapis.com/v1/projects/visioflow-cb6eb-9d051/databases/(default)/documents/site_config/main?key=AIzaSyD2R3SfaC6ifiA_juCfM_1q7SRaAm-G1gY';

function applyAssistancePhones(p1, p2) {
  const s1 = document.getElementById('assist-p1');
  const s2 = document.getElementById('assist-p2');
  const l1 = document.getElementById('assist-p1-lnk');
  const l2 = document.getElementById('assist-p2-lnk');
  const block = document.getElementById('form-assist-block');
  if (s1 && p1) { s1.textContent = p1; if (l1) l1.href = 'tel:' + p1.replace(/\s/g, ''); }
  if (s2 && p2) { s2.textContent = p2; if (l2) l2.href = 'tel:' + p2.replace(/\s/g, ''); }
  if (block) block.style.display = (p1 || p2) ? 'flex' : 'none';
}

(function loadAssistancePhones() {
  fetch(FIRESTORE_ASSIST_URL, { cache: 'no-store' })
    .then(function(r) { return r.ok ? r.json() : null; })
    .then(function(doc) {
      if (!doc || !doc.fields) return;
      const p1 = (doc.fields.assistancePhone1 && doc.fields.assistancePhone1.stringValue) || '';
      const p2 = (doc.fields.assistancePhone2 && doc.fields.assistancePhone2.stringValue) || '';
      applyAssistancePhones(p1, p2);
      // Pre-fill admin inputs if visible
      const i1 = document.getElementById('admin-assist-p1');
      const i2 = document.getElementById('admin-assist-p2');
      if (i1 && p1) i1.value = p1;
      if (i2 && p2) i2.value = p2;
    })
    .catch(function() {});
})();

window.saveAssistancePhones = function() {
  const p1 = (document.getElementById('admin-assist-p1') || {}).value || '';
  const p2 = (document.getElementById('admin-assist-p2') || {}).value || '';
  const msg = document.getElementById('admin-assist-msg');
  if (msg) { msg.style.color = '#60a5fa'; msg.textContent = 'Sauvegarde...'; }

  const body = {
    fields: {
      assistancePhone1: { stringValue: p1.trim() },
      assistancePhone2: { stringValue: p2.trim() }
    }
  };
  fetch(FIRESTORE_ASSIST_URL + '&updateMask.fieldPaths=assistancePhone1&updateMask.fieldPaths=assistancePhone2', {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
  ...
  .then(function(r) {
    ...
    applyAssistancePhones(p1.trim(), p2.trim());
    if (msg) { msg.style.color = '#34d399'; msg.textContent = '✓ Numéros sauvegardés'; setTimeout(function(){ msg.textContent = ''; }, 3000); }
  })
  .catch(function(e) {
