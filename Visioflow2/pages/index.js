...
function renderFormCities(){
  ...
  container.innerHTML=formCities.map((c,i)=>{
    return '<div class="form-city-block">'
      ...
      +'<input class="form-input" type="text" placeholder="Ex: Burger Factory Paris" value="'+c.name+'" onchange="formCities['+i+'].name=this.value"/></div>'
      +'<div class="form-row">'
      +'<div class="form-group"><label class="form-label">Adresse <span class="req">*</span></label>'
      +'<input class="form-input" type="text" placeholder="12 Rue..., 75001 Paris" value="'+c.address+'" onchange="formCities['+i+'].address=this.value"/></div>'
      +'<div class="form-group"><label class="form-label">Téléphone <span class="req">*</span></label>'
      +'<input class="form-input" type="tel" placeholder="01 23 45 67 89" value="'+c.tel+'" onchange="formCities['+i+'].tel=this.value"/></div>'
      +'</div>'
      +'<div class="form-row">'
      +'<div class="form-group"><label class="form-label">Email</label>'
      +'<input class="form-input" type="email" placeholder="contact@..." value="'+c.email+'" onchange="formCities['+i+'].email=this.value"/></div>'
      +'<div class="form-group"><label class="form-label">Horaires</label>'
      +'<input class="form-input" type="text" placeholder="Lun-Sam 12h-22h" value="'+c.horaires+'" onchange="formCities['+i+'].horaires=this.value"/></div>'
      +'</div></div>';
  }).join('');
...
function setMenuMode(mode){
  ...
  if(!itemsZone) return;
  const activeBase = 'flex:1;padding:10px 14px;border-radius:12px;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;transition:all .2s;display:flex;align-items:center;justify-content:center;gap:6px';
  if(mode==='items'){
    itemsZone.style.display=''; photoZone.style.display='none';
    if(btnItems){btnItems.style.cssText=activeBase+';border:1.5px solid var(--blue);background:var(--blue);color:#fff;flex:1';}
    if(btnPhoto){btnPhoto.style.cssText=activeBase+';border:1.5px solid #e5e7eb;background:#fff;color:#6b7280;flex:1';}
  } else {
    itemsZone.style.display='none'; photoZone.style.display='';
    if(btnPhoto){btnPhoto.style.cssText=activeBase+';border:1.5px solid var(--blue);background:var(--blue);color:#fff;flex:1';}
    if(btnItems){btnItems.style.cssText=activeBase+';border:1.5px solid #e5e7eb;background:#fff;color:#6b7280;flex:1';}
  }
}
...
window.handleLogoUpload=function(input){
  ...
  r.onload=function(e){
    ...
    const wrap=logoFile&&logoFile.closest('.form-upload');
    const txt=wrap&&wrap.querySelector('.form-upload-txt');
    if(txt) txt.textContent='Logo chargé — cliquer pour changer';
  };
  r.readAsDataURL(file);
};
window.menuItemPhoto=function(input,cat,i){
  const file=input.files[0]; if(!file) return;
  const r=new FileReader();
...
function formSubmit(){
  ...
  if(firebaseReady && db){
    try{
      db.collection('form_submissions').add({
        ...
        cities: formCities,
        menuItems: menuItems,
        restaurantName: formCities[0]&&formCities[0].name ? formCities[0].name : '',
        timestamp: firebase.firestore.FieldValue.serverTimestamp(),
        status:'new'
      });
  ...
  window.scrollTo({top:0,behavior:'smooth'});
  const payUrl=PAYMENT_LINKS[formPack]&&PAYMENT_LINKS[formPack].stripe;
  if(payUrl) setTimeout(()=>{ window.location.href=payUrl; }, 2000);
}
...
function buildSimHTML(){
  ...
  const heroHtml=
    ...
    +'</div>'
    +'<span style="font-size:8px;color:rgba(255,255,255,.35)">â€“22h30</span>'
    +'</div>'
    +'</div>'
    +'</div>'
    +'</div>';
  ...
  const infoHtml=
    ...
    +'</div>'
    +'<div style="flex:1"><div style="font-size:11px;font-weight:600;color:#fff">01 23 45 67 89</div><div style="font-size:9px;color:rgba(255,255,255,.3)">Lunâ€“Sam · 12hâ€“22h30</div></div>'
  ...
  // â”€â”€ Page: Infos â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    pageHTML=
      ...
      +'<div onclick="sToast(\'ðŸ“ž Appel en cours\u2026\')" style="padding:5px 12px;border-radius:8px;background:'+color+';font-size:10px;font-weight:700;color:#fff;cursor:pointer">Appeler</div></div>'
...
window.goToForm = function(pack){
  ...
  if(nm) nm.textContent=p.name;
  if(pr) pr.textContent=p.price;
  var del=document.getElementById('form-delivery-section');
  if(del) del.style.display=pack==='essentiel'?'none':'';
...
window.formSubmit = window.formSubmit || function(){
  ...
  var s3=document.getElementById('form-step-3');
  var sc=document.getElementById('form-step-success');
  if(s3) s3.style.display='none';
  if(sc) sc.style.display='';
  window.scrollTo({top:0,behavior:'smooth'});
  var url=(typeof PAYMENT_LINKS!=='undefined'&&PAYMENT_LINKS[pack]&&PAYMENT_LINKS[pack].stripe)||({essentiel:'https://buy.stripe.com/8x228kbpzaHz8CHajjdUY07',premium:'https://buy.stripe.com/7sYbIUfFPdTL1afdvvdUY06'}[pack]);
  if(url) setTimeout(function(){window.location.href=url;},2000);
};
    ...
    <div class="cl-card">
      ...
      <div class="cl-form" id="cl-login-form">
        ...
        <div class="cl-field">
          ...
          <div style="position:relative"><input type="password" id="cl-log-pw" placeholder="\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022" onkeydown="if(event.key==='Enter')doClientLogin()" style="width:100%;box-sizing:border-box;padding-right:44px"/><button type="button" onclick="var i=this.parentElement.querySelector('input');if(i){var s=i.type==='password';i.type=s?'text':'password';this.textContent=s?'ðŸ™ˆ':'ðŸ‘ï¸'}" style="position:absolute;right:10px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;font-size:18px;line-height:1;z-index:2;padding:4px">\ud83d\udc41\ufe0f</button></div>
        </div>
      ...
      <div class="cl-form" id="cl-register-form" style="display:none">
        ...
        <div class="cl-field">
          ...
          <div style="position:relative"><input type="password" id="cl-reg-pw" placeholder="\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022" onkeydown="if(event.key==='Enter')doClientRegister()" style="width:100%;box-sizing:border-box;padding-right:44px"/><button type="button" onclick="var i=this.parentElement.querySelector('input');if(i){var s=i.type==='password';i.type=s?'text':'password';this.textContent=s?'ðŸ™ˆ':'ðŸ‘ï¸'}" style="position:absolute;right:10px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;font-size:18px;line-height:1;z-index:2;padding:4px">\ud83d\udc41\ufe0f</button></div>
        </div>
        <div class="cl-field">
          <label>Confirmer</label>
          <div style="position:relative"><input type="password" id="cl-reg-pw2" placeholder="\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022" onkeydown="if(event.key==='Enter')doClientRegister()" style="width:100%;box-sizing:border-box;padding-right:44px"/><button type="button" onclick="var i=this.parentElement.querySelector('input');if(i){var s=i.type==='password';i.type=s?'text':'password';this.textContent=s?'ðŸ™ˆ':'ðŸ‘ï¸'}" style="position:absolute;right:10px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;font-size:18px;line-height:1;z-index:2;padding:4px">\ud83d\udc41\ufe0f</button></div>
        </div>
...
function buildInfoPanelHTML(){
  ...
  let rows = fields.map(f => `
    <div class="vf-info-row">
      ...
      <button class="vf-info-edit" onclick="openEditInfo('${f.key}',\`${f.label}\`,\`${(f.value||'').replace(/`/g,'\\`')}\`)">âœ</button>
    </div>
  `).join('');
...
/* â•”â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•—
   â•‘  REMPLACE ICI PAR TON EMAIL ET TON MOT DE PASSE     â•‘
   â•šâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
const ADMIN_EMAIL    = 'visioflow77@gmail.com';   // â† Mets ton email ici
const ADMIN_PASSWORD = 'Restoflow77@';        // â† Mets ton mot de passe ici
  ...
  <div class="login-container">
    <div class="login-card">
      ...
      <div class="login-form-auth" id="auth-login-form">
        ...
        <div class="login-field">
          <label>Mot de passe</label>
          <div style="position:relative"><input type="password" id="login-password" placeholder="\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022" onkeydown="if(event.key==='Enter')doAdminLogin()" style="width:100%;box-sizing:border-box;padding-right:44px"/><button type="button" onclick="var i=this.parentElement.querySelector('input');if(i){var s=i.type==='password';i.type=s?'text':'password';this.textContent=s?'ðŸ™ˆ':'ðŸ‘ï¸'}" style="position:absolute;right:10px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;font-size:18px;line-height:1;z-index:2;padding:4px">\ud83d\udc41\ufe0f</button></div>
        </div>
...
(function () {
  ...
  _authOv.innerHTML = `
    <div class="vf-auth-card">
      ...
      <div class="vf-auth-form" id="vf-form-log">
        ...
        <div class="vf-af-f"><label class="vf-af-l">Mot de passe</label><div style="position:relative"><input class="vf-af-i" type="password" id="vf-log-pw" placeholder="â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢" onkeydown="if(event.key==='Enter')vfDoLogin()" style="width:100%;box-sizing:border-box;padding-right:44px"/><button type="button" onclick="var i=this.parentElement.querySelector('input');if(i){var s=i.type==='password';i.type=s?'text':'password';this.textContent=s?'ðŸ™ˆ':'ðŸ‘ï¸'}" style="position:absolute;right:10px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;font-size:18px;line-height:1;z-index:2;padding:4px">ðŸ‘ï¸</button></div></div>
  ...
  async function _renderEC () {
    ...
    wrap.innerHTML = `
      <div class="ec-hero">
        ...
        <div><div class="ec-name">Bonjour, ${nm}&nbsp;ðŸ‘‹</div><div class="ec-email">${currentFBUser.email}</div></div>
      </div>
  ...
  async function _loadProjects () {
    ...
    try {
      ...
      card.innerHTML = projects.map(p => {
        ...
        const dt = p.createdAt?.toDate ? p.createdAt.toDate().toLocaleDateString('fr-FR') : 'â€“';
  ...
  window.vfMenuPhotoUpload = function(input) {
    const file = input.files[0]; if (!file) return;
    ...
    reader.onload = function(e) {
      ...
      if (lbl) lbl.textContent = 'Photo chargée — cliquer pour changer';
  ...
  function _renderStep() {
    ...
    document.getElementById('vf-pp-sbar').innerHTML = steps.map((s, i) => {
      ...
      return `<div class="vf-pp-s ${cls}"><div class="vf-pp-snum">${n < _ppStep ? 'âœ“' : n}</div><div class="vf-pp-slbl">${s.lbl}</div></div>`;
    }).join('');
        ...
        <div class="vf-pp-field"><label class="vf-pp-lbl">Horaires d'ouverture</label>
          <div class="pp-hours-grid">
            ${days.map(([k,lbl]) => `<div class="pp-hours-row">
              ...
              <input class="vf-pp-inp pp-hours-inp" id="hr-${k}" placeholder="11h30â€“14h · 19hâ€“22h30" value="${_esc(hrs[k]||'')}"/>
            </div>`).join('')}
          </div>
        </div>
    ...
    foot.innerHTML = `
      ${!isFirst ? '<button class="vf-pp-prev" onclick="vfPPBack()">â† Retour</button>' : '<span></span>'}
      ${isLast
  ...
  window.vfPPSubmit = async function() {
    ...
    if (stripeBase) {
      let qs = [];
      if (clientEmail) qs.push('prefilled_email=' + encodeURIComponent(clientEmail));
      if (docId)       qs.push('client_reference_id=' + encodeURIComponent(docId));
      const url = stripeBase + (qs.length ? (stripeBase.includes('?') ? '&' : '?') + qs.join('&') : '');
      _vfShowRedirectScreen(pack, url);
