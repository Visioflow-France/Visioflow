...
function buildAIPrompt(group, cfg) {
  ...
  // ── EN-TÊTE ──────────────────────────────────────────────────
  lines.push(`Tu es un développeur web expert en Next.js. Crée un PROJET Next.js COMPLET et FONCTIONNEL à 100% pour ce restaurant (Pages Router — pas App Router). Projet multi-fichiers propre : composants réutilisables, données serveur via Firebase Admin SDK, ISR pour la performance. Utilise EXACTEMENT les vraies informations et photos fournies ci-dessous.`)
  lines.push('')
  lines.push(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`)
  ...
  if (isPremium) {
    ...
    lines.push(`- Changements visibles sur le site public au prochain chargement (getDocs + cache, pas d'onSnapshot — voir section Optimisation)`)
    lines.push('')
  ...
  if (isPremium) {
    ...
    lines.push(`- Le site public lit Firebase via getDocs() (chargement unique + cache navigateur) → les changements apparaissent au prochain rechargement`)
    lines.push(`- (Le temps réel via onSnapshot est réservé au dashboard admin, pour préserver les quotas du Firebase partagé — voir section Optimisation)`)
    lines.push(`- Architecture Firebase partagée : même projet VisioFlow, données isolées par restaurant`)
  }
  lines.push('')

  // ── SECTION : TECHNIQUE ──────────────────────────────────────
  lines.push(`## ${s++}. ARCHITECTURE NEXT.JS (Pages Router, multi-fichiers)`)
  lines.push(`Projet Next.js propre, **PAS un fichier HTML unique**. Structure recommandée :`)
  lines.push(`- \`package.json\` : next, react, react-dom, firebase, firebase-admin, sharp, busboy, browser-image-compression`)
  lines.push(`- \`pages/\` : \`index.js\` (accueil), \`carte.js\` (menu complet), \`commander.js\` (plateformes), \`dashboard.js\` (admin), \`api/admin/data.js\`, \`api/admin/upload.js\`, \`api/admin/products.js\`, \`api/admin/config.js\`, \`api/auth/session.js\``)
  lines.push(`- \`lib/\` : \`firebase-admin.js\` (Admin SDK serveur), \`firebase.js\` (Client SDK admin), \`site-data.js\` (getMenu/getSiteConfig), \`storage-utils.js\` (suppression images), \`revalidate.js\` (ISR on-demand), \`auth-session.js\` (cookie session), \`constants.js\``)
  lines.push(`- \`components/\` : Header, Footer, Hero, MenuSection, ProductCard, ProductModal, ImageField (upload compressé), Seo…`)
  lines.push(`- \`public/\` : logo, image hero, favicon`)
  lines.push(`- **Styling** : variables CSS \`--brand: ${color}; --brand-dark; --text; --bg; --bg-alt\` (ou Tailwind). Responsive mobile 320px+ / tablette 768px+ / desktop 1200px+.`)
  lines.push(`- Navigation fixe (backdrop-filter: blur), smooth scroll natif, animations CSS subtiles au scroll (opacity + translateY).`)
  lines.push(`- **SEO LOCAL OBLIGATOIRE — applique chaque point sans exception :**`)
  lines.push(`  • <title> : "${restaurantName}${cuisine ? ` — ${cuisine}` : ''}${address ? ` à ${address.split(',').slice(-2).join(',').trim()}` : ''} | Restaurant"`)
  lines.push(`  • <meta name="description"> : 150-160 caractères, inclut nom + type cuisine + ville + avantage clé (ex: "livraison rapide", "réservation en ligne")`)
  lines.push(`  • <meta name="keywords"> : restaurant + cuisine + ville + quartier + "réservation" + "livraison" + plats phares du menu`)
  lines.push(`  • <meta name="robots" content="index, follow">`)
  lines.push(`  • Open Graph complet : og:title, og:description, og:type="restaurant.restaurant", og:locale="fr_FR", og:image (1ère photo du restaurant)`)
  lines.push(`  • Twitter Card : twitter:card="summary_large_image", twitter:title, twitter:description, twitter:image`)
  lines.push(`  • Schema.org JSON-LD type "Restaurant" COMPLET avec :`)
  lines.push(`    - name, description, image, telephone: "${phone || ''}", email: "${email || ''}"`)
  lines.push(`    - address: PostalAddress avec streetAddress, addressLocality, addressCountry: "FR"`)
  lines.push(`    - openingHoursSpecification (si horaires fournis : ${horaires ? '"' + horaires.substring(0,60) + '..."' : 'non fournis — invente des horaires réalistes'})`)
  lines.push(`    - servesCuisine: "${cuisine || 'Cuisine française'}"`)
  lines.push(`    - priceRange: "€€"`)
  lines.push(`    - hasMenu: true`)
  lines.push(`    - acceptsReservations: "${isPremium ? 'false' : 'true'}"`)
  lines.push(`    - url, sameAs (réseaux sociaux fournis)`)
  lines.push(`  • Toutes les images : attribut alt descriptif avec nom du plat/restaurant + ville`)
  lines.push(`  • Balises sémantiques : 1 seul <h1> (nom restaurant), <h2> pour chaque section, <h3> pour les plats`)
  lines.push(`  • Mentionner la ville et le quartier naturellement dans le contenu (hero, à propos, footer) — pas juste dans les meta`)
  lines.push(`  • Lien Google Maps cliquable sur l'adresse (https://maps.google.com/?q=ADRESSE_ENCODEE)`)
  lines.push(`  • Numéro de téléphone en attribut href="tel:${phone || ''}" partout où il apparaît`)
  lines.push(`- Google Fonts (via next/font ou \`_document.js\`) : Playfair Display + Inter.`)
  lines.push(`- Les pages PUBLIQUES utilisent \`getStaticProps\` + \`revalidate\` (ISR) — voir section Optimisation. \`dashboard.js\` garde \`getServerSideProps\` (cookie de session admin).`)
  lines.push('')

  // ── SECTION : OPTIMISATION + GALÈRES O'77 (CRITIQUE) ─────────
  lines.push(`## ${s++}. ⚠️ OPTIMISATION & GALÈRES ÉVITÉES (leçons du projet O'77)`)
  lines.push(`Tous les restaurants partagent le MÊME projet Firebase. Quotas gratuits : 50 000 lectures Firestore/jour et 5 Go de Storage. Un site mal codé peut TOUT saturer. Applique CES RÈGLES (toutes apprises en production sur O'77) :`)
  lines.push('')
  lines.push(`### A) Lectures Firestore — ISR OBLIGATOIRE`)
  lines.push(`- **Pages publiques (index, carte, commander) : \`getStaticProps\` + \`revalidate: 600\`** (ISR). Ne JAMAIS utiliser \`getServerSideProps\` pour le menu public : sinon ~30 lectures Firestore À CHAQUE visite = saturation immédiate des 50 000/jour.`)
  lines.push(`- Lecture Firestore côté **SERVEUR via Firebase Admin SDK** (\`lib/firebase-admin.js\` → fonction \`getMenu()\`), JAMAIS via le Client SDK sur le site public.`)
  lines.push(`- **ISR on-demand** : après chaque modif du dashboard (produit, catégorie, horaires…), appeler \`res.revalidate('/')\`, \`res.revalidate('/carte')\`, \`res.revalidate('/commander')\`. Centraliser dans \`lib/revalidate.js\` (\`revalidateMenu(res)\`, erreurs non bloquantes).`)
  lines.push(`- Le \`dashboard.js\` reste en \`getServerSideProps\` (cookie de session) et lit via \`/api/admin/data\` → toujours à jour, non concerné par l'ISR.`)
  lines.push('')
  lines.push(`### B) Images — les 5 pièges d'O'77`)
  lines.push(`1. **Photos iPhone = HEIC** → compression côté client OBLIGATOIRE avant l'upload (sinon "Format non supporté"). Utiliser \`browser-image-compression\` dans le composant \`ImageField\` du dashboard : max 1200px, qualité 0.7-0.8, sortie WebP < 150 Ko.`)
  lines.push(`2. **Firebase Storage doit être ACTIVÉ** dans la console (Storage > Commencer ; plan Blaze requis mais 0€ en pratique grâce au quota gratuit). Sinon erreur "The specified bucket does not exist".`)
  lines.push(`3. **Rendre chaque fichier PUBLIC** (\`file.makePublic()\`) après l'upload, sinon l'image ne s'affiche pas ("Image introuvable — vérifiez l'URL"). URL publique au format : \`https://firebasestorage.googleapis.com/v0/b/BUCKET/o/PATH%2Ffichier.webp?alt=media\`.`)
  lines.push(`4. **Nettoyer l'ancienne image** quand un plat/photo est remplacé ou supprimé (\`bucket.file(path).delete()\` via Admin SDK) — sinon fichiers fantômes = saturation des 5 Go. Centraliser dans \`lib/storage-utils.js\` (\`deleteStorageFile(url)\` + parse de l'URL).`)
  lines.push(`5. **Sur serverless (Vercel/Netlify), le système de fichiers est en LECTURE SEULE** → NE JAMAIS écrire dans \`public/uploads/\` (erreur \`ENOENT … /var/task/public\`). Toujours stocker dans Firebase Storage.`)
  lines.push(`- Upload via route \`/api/admin/upload.js\` : \`busboy\` (parse multipart) + \`sharp\` (rotation EXIF + redimensionnement de sécurité) + Admin SDK (\`bucket.file().save()\` + \`makePublic()\`).`)
  lines.push(`- **storage.rules** à coller dans Console Firebase > Storage (filet si la compression est contournée) :`)
  lines.push('```')
  lines.push(`rules_version = '2';`)
  lines.push(`service firebase.storage {`)
  lines.push(`  match /b/{bucket}/o {`)
  lines.push(`    allow read: if true;`)
  lines.push(`    match /restaurants/{allPaths=**} {`)
  lines.push(`      allow write: if request.auth != null && request.resource.size < 2 * 1024 * 1024 && request.resource.contentType.matches('image/.*');`)
  lines.push(`    }`)
  lines.push(`  }`)
  lines.push(`}`)
  lines.push('```')
  ...
  lines.push(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`)
  lines.push(`Livre le PROJET Next.js COMPLET, fichier par fichier : un bloc de code par fichier, précédé de son chemin en commentaire (ex. \`// pages/index.js\`).`)
  lines.push(`Couvre TOUT : \`package.json\`, \`next.config.js\`, \`lib/*\`, \`pages/*\`, \`components/*\`, \`public/\` (logo + hero), \`styles/\`. Aucun fichier générique, abrégé ou manquant.`)
  ...
  if (!isPremium && hasPlateforms) lines.push(`Pack Essentiel : les boutons de livraison (Uber Eats / Deliveroo / Just Eat) doivent être bien visibles et fonctionnels. Aucun système de panier, ni paiement, ni commande directe.`)
  lines.push(`Commence par l'arborescence du projet, puis livre chaque fichier complet. Respecte SCRUPULEUSEMENT la section OPTIMISATION (ISR + 5 pièges images) — c'est ce qui sépare un site qui tient d'un site qui fait exploser les quotas.`)
